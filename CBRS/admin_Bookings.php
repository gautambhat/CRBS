...
color:#13c5ff;}
	...
	<body>
		...
		<?php
			$getBookingQuery = mysql_query("SELECT bid, u_id, bdate, startTime, endTime, username, roomName from booking, login, room where r_id = rid AND u_id = uid order by bdate, startTime") or die(mysql_error());
			while($getBooking = mysql_fetch_array($getBookingQuery))
			{
				echo '<tr><td align="center" >'.$getBooking['bdate'].'</td><td style="width:5em;" align="center">'.$getBooking['startTime'].'hrs</td><td style="width:5em;" align="center">'.$getBooking['endTime'].'hrs</td><td align="center">'.$getBooking['username'].'</td><td align="center">'.$getBooking['roomName'].'</td><td align="center"><a class="cancelButton" title="Click here to cancel this booking" href="admin_Bookings.php?cancelBooking='.$getBooking['bid'].'&userId='.$getBooking['u_id'].'" onclick="return confirm('."'Are you sure you want to cancel this booking?'".')">Cancel?</a></td></tr>';
			}
		?>
		</table>
